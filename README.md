...
- `confirmAction: String | (item | null) -> String | null`: names the package action equivalent to `core:confirm`, so that semantic row displays the user's confirm keybinding without rebinding Enter away from the list chrome. A string applies while a real item is selected; use a function when the command depends on the selected row or an empty selection. The private `Show more…` row never resolves it.
- `additionalActionCommands: [String]`: explicitly includes reachable commands inherited from the panel host or workspace. Their existing command descriptors and keybindings remain the only source of label, description, scope, and shortcut; missing commands are ignored and duplicate names are listed once.
- `actionsFilter: (descriptor) -> Boolean`: narrows which available commands the item-actions list offers. `core:*` and the built-in chrome commands are excluded whatever it returns. On a select list, an item-scoped action is also hidden without a selected item, while `actionScope: "list"` remains available. The list is rebuilt every time it opens with the selection already made, so a predicate that reads the selected item lists an action only for the rows it applies to.
...
- `showItemActions()`: Shows the item-actions list as a modal-flow step (crumb "Actions") — the commands the dialog contributes and any host commands named by `additionalActionCommands`, with the label, description, and keybindings each carries in the command registry and keymaps, rendered command-palette style. A `confirmAction` row additionally displays the resolved `core:confirm` binding, deduplicated against its own shortcuts, while Enter itself remains list chrome. While at least one action is available, an ellipsis button in the query editor opens the list; `select-list:actions` opens it from the master and returns from the actions list, so the command toggles between them. Confirming a row — or dispatching one of the action commands from within the actions list, which wears the master's classes so the package keymap applies there untouched — returns to the master first and then dispatches the command, exactly as if it were invoked there.
