 ...
 * directly for dialogs that are not lists (prompts, save dialogs, forms) and
 * host arbitrary DOM through the `contentElement` prop.
 */
class InputDialogView {
   ...
   * new target lives inside `contentElement` (interactive custom content).
   * If focus moves outside, cancels after a frame delay.
  ...
  didLoseFocus(event) {
    ...
    if (this.element.contains(event.relatedTarget)) {
      ...
      if (this.isTargetInContent(event.relatedTarget)) return;
      this.refs.queryEditor.element.focus();
      return;
    }
   ...
   * Interactive elements inside `contentElement` are exempt so custom dialog
   * content (inputs, buttons) can receive focus and clicks normally.
  ...
  didMouseDownOnElement(event) {
    ...
    if (interactive && this.isTargetInContent(interactive)) return;
  ...
  /**
   * Returns whether a node lives inside the custom content element.
   * @param {Node} node - The node to test
   * @returns {boolean} True when `contentElement` is set and contains the node
   */
  isTargetInContent(node) {
    return !!(this.props.contentElement && node && this.props.contentElement.contains(node));
  }
