...
class InputDialogView {
  ...
  show(options) {
    ...
    // whose trail was abandoned (F12, then Escape) from surviving into the
   ...
   * back from a flow step — F12 into the actions list and back — which is a
   ...
   * chrome (`core:*`, `select-list:*` built-ins) stays out, always. An
   ...
   * is selected — the list is rebuilt on every F12, with the selection
  ...
  itemActions() {
    ...
    // caller says; `actionsFilter` only narrows what survives them.
   ...
   * refresh that dropped it has genuinely unselected it.
  ...
  runItemAction(command) {
    ...
    if (selected != null && this.items?.includes(selected)) {
