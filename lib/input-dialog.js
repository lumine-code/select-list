...
class InputDialogView {
   ...
   * the show side effects (help reset, willShow, select-all, focus) also run
  ...
  computeHelp() {
    if (this.props.helpMessage) {
      ...
      if (atom.tools && atom.tools.markdown && atom.tools.markdown.render) {
        this.helpMessage = atom.tools.markdown.render(this.props.helpMarkdown);
      } else {
