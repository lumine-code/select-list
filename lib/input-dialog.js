...
class InputDialogView {
  ...
  show(options) {
    ...
    const panelOptions = Object.hasOwn(showOptions, "crumb")
      ...
      : undefined;
    this.getPanel().show(panelOptions);
  }
  ...
  // The primary modal container may adopt a dialog constructed from another
  // live Document. Subclasses rebuild browser objects whose implicit root
  // belongs to the Window that constructed them. Etch itself follows
  // element.ownerDocument on every update, so ordinary DOM needs no reset.
  didChangeDocument() {}
  ...
  /**
   * Returns the stable primary-workspace modal panel that hosts the dialog,
   * creating it (hidden) on first access. The panel's item is `props.panelItem`
   * when provided, otherwise the dialog itself. `props.owner` is passed as a
   * logical lifecycle context. The panel carries the dialog's `crumb` prop as
   * its declared breadcrumb label.
  ...
  getPanel() {
    if (!this.panel) {
      ...
      // addModalPanel inserts the item into the primary modal container. If a
      // caller constructed it in another live realm, synchronize all later
      // rendering with the Document that now owns the stable panel item.
  ...
  /**
   * Toggles the visibility of the dialog, passing presentation options to
