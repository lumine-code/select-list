...
class InputDialogView {
  ...
  constructor(props) {
    ...
    if (!this.document?.defaultView) {
      throw new TypeError("InputDialogView requires a live Document");
    }
   ...
   */
  didLoseFocus(event) {
    if (this.panel?.isTransferring()) return;
   ...
   *
   * @param {Object} [options] - One-show presentation options.
   * @param {String|Boolean} [options.crumb] - A label, or `true` to use the
   *   dialog's declared crumb, presenting this show as a modal-flow step.
   * @param {*} [options.owner] - Pane item whose current surface owns this
   *   show. A dialog with a constructor-level route rejects a show-level route.
   * @param {*} [options.surface] - Explicit surface for this show. A dialog
   *   with a constructor-level route rejects a show-level route.
   */
  show(options) {
    const showOptions = options || {};
    for (const key of Object.keys(showOptions)) {
      if (key !== "crumb" && key !== "owner" && key !== "surface") {
        throw new TypeError(`InputDialogView::show received an unknown option "${key}"`);
      }
    }
    if (
      Object.hasOwn(showOptions, "crumb") &&
      typeof showOptions.crumb !== "string" &&
      showOptions.crumb !== true
    ) {
      throw new TypeError("The crumb option must be a string or true");
    }

    const route = this.modalRouteForShow(showOptions);
    const panel = this.getPanel();
    lumine.workspace.rehomeModalPanel(panel, route);
    this.updateDocumentFromElement();
    // An explicit successful show is always an opening, never a resume. The
    // flow re-shows a step through the panel rather than through here, so this
    // only clears a suspension whose trail was abandoned before a fresh open.
    this.suspendedByFlow = false;
    const panelOptions = Object.hasOwn(showOptions, "crumb")
      ? { crumb: showOptions.crumb }
      : undefined;
    panel.show(panelOptions);
  }

  modalRouteForShow(options = {}) {
    if (this.props.surface != null && this.props.owner != null) {
      throw new TypeError("A dialog accepts either a declared owner or surface, not both");
    }
    if (options.surface != null && options.owner != null) {
      throw new TypeError("A dialog show accepts either owner or surface, not both");
    }
    const hasDeclaredRoute = this.props.surface != null || this.props.owner != null;
    const hasShowRoute = options.surface != null || options.owner != null;
    if (hasDeclaredRoute && hasShowRoute) {
      throw new TypeError("A show-level modal route cannot override the dialog's declared route");
    }
    if (hasDeclaredRoute) {
      return this.props.surface != null
        ? { surface: this.props.surface }
        : { owner: this.props.owner };
    }
    if (hasShowRoute) {
      return options.surface != null ? { surface: options.surface } : { owner: options.owner };
    }
    return {};
  }

  updateDocumentFromElement() {
    const nextDocument = this.element.ownerDocument;
    if (nextDocument === this.document) return false;
    const previousDocument = this.document;
    this.document = nextDocument;
    this.didChangeDocument(previousDocument, nextDocument);
    return true;
  }

  // Subclasses rebuild browser objects whose implicit root belongs to the
  // Window that constructed them. Etch itself follows element.ownerDocument
  // on every update, so ordinary rendered DOM needs no corresponding reset.
  didChangeDocument() {}
  ...
  /**
   * Returns the stable modal panel that hosts the dialog, creating it (hidden)
   * on first access. Explicit shows may move this same Panel between surface
   * containers. The panel's item is `props.panelItem` when provided, otherwise
   * the dialog itself. The panel carries the dialog's `crumb` prop as its
   * declared breadcrumb label.
  ...
  getPanel() {
    if (!this.panel) {
      ...
      this.disposables.add(this.panel.onDidChangeDocument(() => this.updateDocumentFromElement()));
      this.updateDocumentFromElement();
  ...
  /**
   * Toggles the visibility of the dialog, passing one-show routing options to
   * {@link #show} when opening it.
   */
  toggle(options) {
    if (this.isVisible()) {
      this.hide();
  ...
  async showItemActions() {
    ...
    this.itemActionsList.show({
      crumb: "Actions",
      surface: lumine.workspace.getWindowSurface(this.element),
    });
  }
