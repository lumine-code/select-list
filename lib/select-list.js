...
class SelectListView extends InputDialogView {
  ...
  initializeVisibilityObserver() {
    ...
    const IntersectionObserver = this.document.defaultView.IntersectionObserver;
    const observer = new IntersectionObserver((changes) => {
      if (this.visibilityObserver !== observer) return;
  ...
  didChangeDocument(previousDocument, document) {
    super.didChangeDocument(previousDocument, document);
    if (!this.props.initiallyVisibleItemCount) return;
    this.initializeVisibilityObserver();
    this.observeDeferredItems();
  }

  observeDeferredItems() {
    if (!this.visibilityObserver || !this.refs?.items || !this.listItems) return;
    const observer = this.visibilityObserver;
    this.listItems
      .slice(this.props.initiallyVisibleItemCount)
      .forEach((listItem) => observer.observe(listItem.component.element));
  }
  ...
  renderItems() {
    if (this.items && this.items.length > 0) {
      ...
      this.listItems = this.items.map((item, index) => {
        ...
        const opts = { selected, index, filterKey, visible, document: this.document };
        ...
        opts.highlight = (text, indices = opts.matchIndices) =>
          highlightMatches(text, indices, { document: this.document });
        return $(ListItemView, {
      ...
      if (this.visibilityObserver) {
        ...
        etch.getScheduler(this.document).updateDocument(() => {
  ...
  buildCandidates() {
    ...
    if (this.filterMatcher) {
      lumine.tools.fuzzyMatcher.setCandidates(this.filterMatcher, this.candidates);
    } else {
      this.filterMatcher = lumine.tools.fuzzyMatcher.setCandidates(this.candidates, {
  ...
  getMatchIndices(item, filterKey) {
    ...
    if (!this.indexMatcher) {
      this.indexMatcher = lumine.tools.fuzzyMatcher.setCandidates([filterKey], {
        ignoreDiacritics: !!this.props.removeDiacritics,
      });
    } else {
      lumine.tools.fuzzyMatcher.setCandidates(this.indexMatcher, [filterKey]);
    }
  ...
  resolveElement(item, opts) {
    ...
    if (item === SHOW_MORE_ITEM) {
      return createTwoLineItem(
        { primary: "Show more…", className: "show-more-item" },
        this.document,
      );
    }
    const result = this.props.elementForItem(item, opts);
    if (lumine.dom.isElement(result)) {
      // ListItemView schedules work immediately in the row's owner Document.
      // Put a caller-owned row in the list's realm before constructing that
      // component, rather than relying on the later DOM insertion to adopt it.
      if (result.ownerDocument !== this.document) this.document.adoptNode(result);
      return result;
    }
    const element = createTwoLineItem(result, this.document);
  ...
  renderItemAtIndex(index) {
    ...
    const selected = this.selectedItemRaw() === item;
    const filterKey = this.getFilterKey(item);
    const opts = { selected, index, filterKey, visible: true, document: this.document };
    // Lazy getter - matchIndices only computed when accessed
    Object.defineProperty(opts, "matchIndices", {
      get: () => this.getMatchIndices(item, filterKey),
      enumerable: true,
  ...
  selectIndex(index, updateComponent = true) {
    ...
    if (updateComponent) {
      if (this.listItems) {
        ...
        return etch.getScheduler(this.document).getNextUpdatePromise();
      } else {
...
class ListItemView {
  constructor(props) {
    ...
    etch
      .getScheduler(this.element.ownerDocument)
      .updateDocument(this.scrollIntoViewIfNeeded.bind(this));
  }
