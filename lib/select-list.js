...
class SelectListView extends InputDialogView {
   ...
   * row was — and the scroller keeps its position, so neither a click nor
   * Enter makes the list jump.
  ...
  async showMore() {
    ...
    // The ol persists across the update, but the selection's own
    // scroll-into-view runs in the same cycle; put the viewport back last.
    if (this.refs.items) {
